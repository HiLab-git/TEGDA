...
def update_and_predict(net_list, mean_list, std_list, val_imgs, checkpoint_list=None, rho=0.05):
    ...
    for idx in range(len(net_list)):
        ...
        print(f"image requires_grad: {val_imgs.requires_grad}")  # 应该为 True
        print(f"Output requires_grad: {mask_pred.requires_grad}")  # 应该为 True
        print(f"Loss requires_grad: {loss.requires_grad}", loss,'6767')      # 应该为 True
        loss.backward()
...
class TTA(nn.Module):
    def __init__(self, model, anchor_model, repeat_num = 1, steps=1, episodic=False):
        super().__init__()
        self.steps = steps
    ...
    def forward_and_adapt(self, val_imgs):
        ...
        for stats_idx, (input_mean_list, input_std_list) in enumerate(zip(input_means, input_stds)):
            ...
            pred_list, conf_list, sharpness_list = update_and_predict(self.net_list, input_mean_list, input_std_list, self.x, \
                                                                                checkpoint_list=self.anchor_model, rho=0.1)
            # outout = update_and_predict(self.net_list, input_mean_list, input_std_list, self.x, \
            #                                                                     checkpoint_list=self.anchor_model, rho=0.1)
        ...
        tmp_pred = torch.softmax(tmp_pred_raw, 1)
