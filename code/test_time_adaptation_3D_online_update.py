...
from sota.img_update import ImgUpdate
...
def test_single_case(net, image):
    image = image.cuda()
    image = image.float()
    with torch.no_grad():
        y1 = net(image)
...
def test_single_case_mt(net, img, updated_img):
    img = img.cuda().float()
    updated_img = updated_img.cuda().float()
    with torch.no_grad():
        y1 = net(img, updated_img)
        y = torch.argmax(y1, dim=1)
    label = y.cpu().numpy()[0] 
    # print(label.shape)
    # score_map /= np.expand_dims(cnt, axis=0)  # 平均化
...
def train(args, snapshot_path):
    ...
    results = []
    aetta_eval = AETTA()
    # img_update = ImgUpdate(model)
    img_update = ImgHistUpdate()
    # aetta4seg_eval = AETTA4Seg()
    # 把eval_model放出来，相当于始终用source model进行测试
    ...
    for i_batch, sampled_batch in enumerate(trainloader):
        ...
        pred = test_no_adapt(model, volume_batch)
        
        # 进行模型的在线测试
        eval_model = copy.deepcopy(model)
        eval_model.eval()
        est_WT, est_TC, est_ET, est_avg = aetta_eval.aetta(input=volume_batch,pred=pred,model=eval_model)
        print((f'AETTA Dice: WT-{est_WT},TC-{est_TC},ET-{est_ET},Avg-{est_avg}'))
        case_result['aetta_WT'] = est_WT
        case_result['aetta_TC'] = est_TC
        case_result['aetta_ET'] = est_ET
        case_result['aetta_Avg'] = est_avg
        
        # 利用在线测试得到的结果进行图像层面的对齐
        # updated_volume_batch = img_update.img_update(model, volume_batch, pred, est_WT, est_TC, est_ET)
        updated_volume_batch = img_update.img_update(volume_batch, pred, est_WT, est_TC, est_ET)
        # 仅针对肿瘤部分的bounding box进行直方图匹配
        # updated_volume_batch = img_update.img_update_for_tumor(volume_batch, pred, est_WT, est_TC, est_ET)
        
        # final_pred = test_single_case(model, updated_volume_batch)
        # 利用在线测试得到的结果进行图像层面的对齐,同时利用到mean_teacher架构进行
        final_pred = test_single_case_mt(model, volume_batch, updated_volume_batch)
        
        # 针对不同的类别，分别进行图像的update
        # final_pred = img_update.img_update_with_class(model, volume_batch, pred, est_WT, est_TC, est_ET)
        
        case_result.update(online_evaluation(name,label_batch[0][0],final_pred))
        
        ###################### save output ##################
        results.append(case_result)
        test_save_path = os.path.join(save_output_dir, sampled_batch['name'][0])
        prd_itk = sitk.GetImageFromArray(final_pred.astype(np.float32))
